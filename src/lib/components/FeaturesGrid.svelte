...
<section id="why-us" class="py-20 border-b-2 border-navy">
	<div class="container-custom">
		...
		<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
			{#each features as feature, index}
				<div class={[
					'group space-y-4 border-l-2 border-gold pl-6 transition-all duration-700',
					'hover:translate-x-2',
					cardsVisible[index] ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'
				]} style={`transition-delay: ${index * 100}ms`}>
					<div class="w-12 h-12 border-2 border-navy flex items-center justify-center bg-cream transition-all duration-300 group-hover:border-gold group-hover:scale-110">
						<svg class="w-6 h-6 text-navy transition-colors duration-300 group-hover:text-gold" fill="none" stroke="currentColor" viewBox="0 0 24 24">
